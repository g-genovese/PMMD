"""
e8_percolation_implicit_v42.py
==============================
v4.2 extension of e8_percolation_implicit.py.

DROP-IN REPLACEMENT: same CLI, same vertex set, same neighbour structure,
same kernel logic. ADDS incremental tracking of:
  - n_clusters: total number of active clusters
  - sum_s_log_s: sum over clusters of s * log(s)

This enables computation of 3 additional p_c estimators from entropy curves:
  - S1 = log(n_active) - sum_s_log_s / n_active   [Shannon entropy by site]
  - S2 = log(n_clusters)                          [Shannon by cluster count]
  - S3 = 2*log(n_active) - log(sum_sq)            [Renyi-2 entropy by site]

All 5 estimators are returned: pc_chi, pc_dS_max, pc_dS1, pc_dS2, pc_dS3.

Memory cost: 2 extra binned arrays of n_bins floats each = ~160 KB extra
per worker. Negligible compared to the ~10 GB UF state.

Speed cost: 3 log() calls and a few multiplications per cluster merge.
Total cost: O(N) extra log calls. For L=12: ~50 seconds extra per trial,
about 1% overhead.

USAGE (identical to original):
  python e8_percolation_implicit_v42.py --L 12 --trials 128 --workers 14 \\
      --seed 20260516 --save-curves \\
      --output e8_percolation_L12_v42_128trials.json
"""

import argparse
import json
import sys
import time
import platform
import numpy as np
from multiprocessing import cpu_count, get_context

try:
    from numba import njit
    HAS_NUMBA = True
except ImportError:
    HAS_NUMBA = False
    def njit(*args, **kwargs):
        if len(args) == 1 and callable(args[0]):
            return args[0]
        def decorator(f):
            return f
        return decorator


# =====================================================================
# Precompute E_8 root shifts (identical to v1)
# =====================================================================

def make_e8_shifts():
    typeA = []
    for i in range(8):
        for j in range(i + 1, 8):
            for s1 in (1, -1):
                for s2 in (1, -1):
                    d = np.zeros(8, dtype=np.int8)
                    d[i] = s1
                    d[j] = s2
                    typeA.append(d)
    typeA = np.stack(typeA)
    assert typeA.shape == (112, 8)

    typeB_h0 = []
    typeB_h1 = []
    for mask in range(256):
        if bin(mask).count("1") % 2 != 0:
            continue
        s = np.array([-1 if (mask >> k) & 1 else 1 for k in range(8)],
                     dtype=np.int8)
        d0 = ((s - 1) // 2).astype(np.int8)
        d1 = ((s + 1) // 2).astype(np.int8)
        typeB_h0.append(d0)
        typeB_h1.append(d1)
    typeB_h0 = np.stack(typeB_h0)
    typeB_h1 = np.stack(typeB_h1)
    assert typeB_h0.shape == (128, 8)
    assert typeB_h1.shape == (128, 8)
    return typeA, typeB_h0, typeB_h1


# =====================================================================
# Numba kernels
# =====================================================================

@njit(cache=True, fastmath=True)
def _build_order(L, seed):
    L8 = L ** 8
    order = np.empty(L8 + 2, dtype=np.int64)
    pos = 0
    for h in range(2):
        for lex_idx in range(L8):
            x = lex_idx
            s = 0
            for k in range(8):
                s += x % L
                x //= L
            if s % 2 == 0:
                order[pos] = h * L8 + lex_idx
                pos += 1
    order = order[:pos]
    np.random.seed(seed)
    np.random.shuffle(order)
    return order


@njit(cache=True, fastmath=True)
def _newman_ziff_kernel_v42(L, order, typeA, typeB_h0, typeB_h1, n_bins):
    """
    Newman-Ziff Monte Carlo with implicit adjacency.
    v4.2: ALSO tracks n_clusters and sum_s_log_s for entropy estimators.
    Returns (S_max_bin, sum_sq_bin, n_clusters_bin, sum_s_log_s_bin).
    """
    L8 = L ** 8
    N_idx = 2 * L8
    N_valid = order.shape[0]

    parent = np.full(N_idx, -1, dtype=np.int32)
    size = np.zeros(N_idx, dtype=np.int32)

    largest = np.int32(0)
    sum_sq = np.int64(0)
    n_clusters = np.int64(0)        # NEW
    sum_s_log_s = np.float64(0.0)   # NEW

    bin_size = max(N_valid // n_bins, 1)
    S_max_bin = np.zeros(n_bins, dtype=np.float64)
    sum_sq_bin = np.zeros(n_bins, dtype=np.float64)
    n_clusters_bin = np.zeros(n_bins, dtype=np.float64)   # NEW
    sum_s_log_s_bin = np.zeros(n_bins, dtype=np.float64)  # NEW

    c = np.empty(8, dtype=np.int64)

    for i in range(N_valid):
        v = order[i]
        if v >= L8:
            h = 1
            lex_v = v - L8
        else:
            h = 0
            lex_v = v

        x = lex_v
        for k in range(8):
            c[k] = x % L
            x //= L

        # Activate v
        parent[v] = v
        size[v] = 1
        sum_sq += 1
        n_clusters += 1                    # NEW: new singleton cluster
        # sum_s_log_s += 1 * log(1) = 0     # NEW: no change for singleton
        if largest < 1:
            largest = 1
        current = np.int64(v)

        # ---- Type-A neighbours (112), same half ----
        for ri in range(112):
            lex_u = np.int64(0)
            pw = np.int64(1)
            for k in range(8):
                ck = c[k] + typeA[ri, k]
                if ck < 0:
                    ck += L
                elif ck >= L:
                    ck -= L
                lex_u += ck * pw
                pw *= L
            u = h * L8 + lex_u

            if parent[u] == -1:
                continue

            ra = current
            while parent[ra] != ra:
                ra = parent[ra]
            x2 = current
            while parent[x2] != ra:
                nxt = parent[x2]
                parent[x2] = ra
                x2 = nxt

            rb = u
            while parent[rb] != rb:
                rb = parent[rb]
            x2 = u
            while parent[x2] != rb:
                nxt = parent[x2]
                parent[x2] = rb
                x2 = nxt

            if ra == rb:
                continue

            sa = size[ra]
            sb = size[rb]
            sum_sq += 2 * np.int64(sa) * np.int64(sb)

            # NEW: update sum_s_log_s for the merge
            # Remove sa*log(sa) + sb*log(sb), add (sa+sb)*log(sa+sb)
            new_size = sa + sb
            fsa = np.float64(sa)
            fsb = np.float64(sb)
            fnew = np.float64(new_size)
            sum_s_log_s -= fsa * np.log(fsa)
            sum_s_log_s -= fsb * np.log(fsb)
            sum_s_log_s += fnew * np.log(fnew)
            n_clusters -= 1                # NEW: two clusters became one

            if sa < sb:
                parent[ra] = rb
                size[rb] = sa + sb
                current = rb
                if sa + sb > largest:
                    largest = sa + sb
            else:
                parent[rb] = ra
                size[ra] = sa + sb
                current = ra
                if sa + sb > largest:
                    largest = sa + sb

        # ---- Type-B neighbours (128), cross-half ----
        if h == 0:
            typeB = typeB_h0
            h_new = 1
        else:
            typeB = typeB_h1
            h_new = 0

        for ri in range(128):
            lex_u = np.int64(0)
            pw = np.int64(1)
            for k in range(8):
                ck = c[k] + typeB[ri, k]
                if ck < 0:
                    ck += L
                elif ck >= L:
                    ck -= L
                lex_u += ck * pw
                pw *= L
            u = h_new * L8 + lex_u

            if parent[u] == -1:
                continue

            ra = current
            while parent[ra] != ra:
                ra = parent[ra]
            x2 = current
            while parent[x2] != ra:
                nxt = parent[x2]
                parent[x2] = ra
                x2 = nxt

            rb = u
            while parent[rb] != rb:
                rb = parent[rb]
            x2 = u
            while parent[x2] != rb:
                nxt = parent[x2]
                parent[x2] = rb
                x2 = nxt

            if ra == rb:
                continue

            sa = size[ra]
            sb = size[rb]
            sum_sq += 2 * np.int64(sa) * np.int64(sb)

            # NEW: update sum_s_log_s for the merge
            new_size = sa + sb
            fsa = np.float64(sa)
            fsb = np.float64(sb)
            fnew = np.float64(new_size)
            sum_s_log_s -= fsa * np.log(fsa)
            sum_s_log_s -= fsb * np.log(fsb)
            sum_s_log_s += fnew * np.log(fnew)
            n_clusters -= 1                # NEW

            if sa < sb:
                parent[ra] = rb
                size[rb] = sa + sb
                current = rb
                if sa + sb > largest:
                    largest = sa + sb
            else:
                parent[rb] = ra
                size[ra] = sa + sb
                current = ra
                if sa + sb > largest:
                    largest = sa + sb

        # Sample at bin boundary
        if (i + 1) % bin_size == 0:
            idx = (i + 1) // bin_size - 1
            if 0 <= idx < n_bins:
                S_max_bin[idx] = float(largest)
                sum_sq_bin[idx] = float(sum_sq)
                n_clusters_bin[idx] = float(n_clusters)       # NEW
                sum_s_log_s_bin[idx] = sum_s_log_s             # NEW

    return S_max_bin, sum_sq_bin, n_clusters_bin, sum_s_log_s_bin


# =====================================================================
# Worker
# =====================================================================

def _worker(args):
    L, seed, typeA, typeB_h0, typeB_h1, n_bins = args
    t0 = time.time()
    order = _build_order(L, seed)
    S_max, sum_sq, n_clusters, sum_s_log_s = _newman_ziff_kernel_v42(
        L, order, typeA, typeB_h0, typeB_h1, n_bins)
    return S_max, sum_sq, n_clusters, sum_s_log_s, time.time() - t0


# =====================================================================
# Analysis (extended with 3 entropy estimators)
# =====================================================================

def analyze_v42(S_max_arr, sum_sq_arr, n_clusters_arr, sum_s_log_s_arr,
                L, n_bins):
    """
    Compute all 5 p_c estimators:
      pc_chi    : peak of susceptibility chi(p)
      pc_dS_max : peak of dS_max/dp (original order parameter derivative)
      pc_dS1    : peak of |dS1/dp| where S1 = Shannon-by-site entropy
      pc_dS2    : peak of |dS2/dp| where S2 = log(n_clusters)
      pc_dS3    : peak of |dS3/dp| where S3 = Renyi-2 entropy
    """
    N = L ** 8
    bin_size = N // n_bins
    ps = np.arange(1, n_bins + 1) * (bin_size / N)

    S_max_mean = S_max_arr.mean(axis=0)
    sum_sq_mean = sum_sq_arr.mean(axis=0)
    n_clusters_mean = n_clusters_arr.mean(axis=0)
    sum_s_log_s_mean = sum_s_log_s_arr.mean(axis=0)

    n_active = (np.arange(1, n_bins + 1) * bin_size).astype(np.float64)

    # Original susceptibility (excluding largest cluster)
    denom = n_active - S_max_mean
    chi = np.where(denom > 0, (sum_sq_mean - S_max_mean ** 2) / denom, 0.0)

    # Original order parameter derivative
    dS_max = np.diff(S_max_mean)

    # NEW: S1 = -sum (s/N) log(s/N) = log(N) - sum_s_log_s / N
    # Non-monotonic: rises from 0, peaks near p_c, falls when spanning cluster dominates
    with np.errstate(divide='ignore', invalid='ignore'):
        S1 = np.where(n_active > 0,
                      np.log(n_active) - sum_s_log_s_mean / n_active,
                      0.0)

    # NEW: S2 = log(n_clusters)
    # Non-monotonic: rises from 0 (n_clusters grows as sites activate), peaks at the p
    # where merging rate equals activation rate, falls as merging dominates
    with np.errstate(divide='ignore', invalid='ignore'):
        S2 = np.where(n_clusters_mean > 0, np.log(n_clusters_mean), 0.0)

    # NEW: S3 = -log(sum (s/N)^2) = 2 log(N) - log(sum_sq)
    # Non-monotonic: similar shape to S1, peak near p_c
    with np.errstate(divide='ignore', invalid='ignore'):
        S3 = np.where(sum_sq_mean > 0,
                      2 * np.log(n_active) - np.log(sum_sq_mean),
                      0.0)

    # Peak locations
    pc_chi = float(ps[int(np.argmax(chi))])
    pc_dS_max = float(ps[int(np.argmax(dS_max))])
    # Entropies S1, S2, S3 are non-monotonic but bounded; argmax of the curve
    # itself gives a clean p_c estimator (no small-p divergence -- that only
    # affects |dS/dp|, not S itself).
    pc_S1 = float(ps[int(np.argmax(S1))])
    pc_S2 = float(ps[int(np.argmax(S2))])
    pc_S3 = float(ps[int(np.argmax(S3))])

    return {
        "pc_chi": pc_chi,
        "pc_dS_max": pc_dS_max,
        "pc_S1": pc_S1,
        "pc_S2": pc_S2,
        "pc_S3": pc_S3,
        "chi": chi,
        "S_max_mean": S_max_mean,
        "n_clusters_mean": n_clusters_mean,
        "sum_s_log_s_mean": sum_s_log_s_mean,
        "S1": S1,
        "S2": S2,
        "S3": S3,
        "ps": ps,
    }


# =====================================================================
# Driver
# =====================================================================

def run_single_L(L, trials, workers, seed_base, n_bins, output, save_curves):
    print("=" * 72)
    print(f"  L = {L}, trials = {trials}, workers = {workers}, bins = {n_bins}")
    print(f"  v4.2 extended kernel: tracks n_clusters and sum_s_log_s")
    print("=" * 72)

    typeA, typeB_h0, typeB_h1 = make_e8_shifts()
    print(f"  Roots: 112 type-A + 128 type-B = {112 + 128} ok")

    N = L ** 8
    mem_per_worker_gb = (2 * N * 4 * 2 + N * 8) / 1e9
    print(f"  N = {N:,} valid vertices (ID space 2N = {2*N:,})")
    print(f"  Memory per worker: ~{mem_per_worker_gb:.2f} GB")
    print(f"  Memory total:      ~{workers * mem_per_worker_gb:.2f} GB ({workers} workers)")
    if workers * mem_per_worker_gb > 165:  # 190 GB system, ~25 GB headroom
        rec = max(1, int(160 / mem_per_worker_gb))
        print(f"  WARNING: total memory may exceed 165 GB. "
              f"Consider --workers {rec}")

    seeds = [seed_base + i for i in range(trials)]
    work = [(L, s, typeA, typeB_h0, typeB_h1, n_bins) for s in seeds]

    t0 = time.time()
    print(f"\n  Launching {trials} trials across {workers} workers...")
    if HAS_NUMBA:
        print(f"  (First trial per worker incurs ~5-10 s numba JIT cost)")
    print(f"  Start: {time.strftime('%Y-%m-%d %H:%M:%S')}")

    ctx = get_context("spawn")
    results = []
    with ctx.Pool(workers) as pool:
        for i, res in enumerate(pool.imap_unordered(_worker, work)):
            results.append(res)
            elapsed = time.time() - t0
            done = i + 1
            ETA = elapsed * (trials - done) / done if done > 0 else 0
            print(f"    trial {done:>3}/{trials} in {res[4]:6.1f}s  "
                  f"(elapsed {elapsed:.0f}s, ETA {ETA:.0f}s)", flush=True)
    dt = time.time() - t0

    S_max_arr = np.stack([r[0] for r in results])
    sum_sq_arr = np.stack([r[1] for r in results])
    n_clusters_arr = np.stack([r[2] for r in results])
    sum_s_log_s_arr = np.stack([r[3] for r in results])
    times = [r[4] for r in results]

    print(f"\n  Total wall time: {dt:.1f}s ({dt/60:.1f} min)")
    print(f"  Per-trial: avg {np.mean(times):.1f}s "
          f"(min {min(times):.1f}, max {max(times):.1f})")

    ana = analyze_v42(S_max_arr, sum_sq_arr, n_clusters_arr, sum_s_log_s_arr,
                      L, n_bins)

    bethe = 1.0 / 239
    fw = 1.0 / 183
    pc_list = [ana['pc_chi'], ana['pc_dS_max'], ana['pc_S1'], ana['pc_S2'], ana['pc_S3']]
    spread_pct = (max(pc_list) - min(pc_list)) / np.mean(pc_list) * 100

    print()
    print(f"  Results L={L}, N={N}:")
    print(f"    p_c estimators (and 1/p_c):")
    print(f"      chi peak              = {ana['pc_chi']:.6f}   1/p_c = {1/ana['pc_chi']:.2f}")
    print(f"      dS_max/dp peak        = {ana['pc_dS_max']:.6f}   1/p_c = {1/ana['pc_dS_max']:.2f}")
    print(f"      S1 peak (Shannon-site)    = {ana['pc_S1']:.6f}   1/p_c = {1/ana['pc_S1']:.2f}")
    print(f"      S2 peak (Shannon-count)   = {ana['pc_S2']:.6f}   1/p_c = {1/ana['pc_S2']:.2f}")
    print(f"      S3 peak (Renyi-2)         = {ana['pc_S3']:.6f}   1/p_c = {1/ana['pc_S3']:.2f}")
    print(f"    Bethe 1/239: {bethe:.6f}")
    print(f"    Target 1/183: {fw:.6f}")
    print(f"    Estimator spread (max-min)/mean: {spread_pct:.2f}%")

    result = {
        "L": L,
        "N": int(N),
        "avg_degree": 240.0,
        "trials": trials,
        "pc_chi": ana["pc_chi"],
        "pc_dS": ana["pc_dS_max"],          # kept for backward compatibility
        "pc_dS_max": ana["pc_dS_max"],
        "pc_S1": ana["pc_S1"],              # NEW: Shannon-by-site entropy peak
        "pc_S2": ana["pc_S2"],              # NEW: Shannon-by-cluster-count peak (= n_clusters peak)
        "pc_S3": ana["pc_S3"],              # NEW: Renyi-2 entropy peak
        "estimator_spread_pct": float(spread_pct),
        "bethe": bethe,
        "target": fw,
        "n_bins": n_bins,
        "total_time_s": dt,
        "method": "implicit_adjacency_newman_ziff_v42",
    }

    if output:
        with open(output, "w") as f:
            json.dump(result, f, indent=2)
        print(f"\n  Saved to {output}")

    if save_curves and output:
        npz = output.replace(".json", "_curves.npz")
        np.savez_compressed(
            npz,
            ps=ana["ps"],
            chi=ana["chi"],
            S_max_mean=ana["S_max_mean"],
            n_clusters_mean=ana["n_clusters_mean"],
            sum_s_log_s_mean=ana["sum_s_log_s_mean"],
            S1=ana["S1"], S2=ana["S2"], S3=ana["S3"],
            S_max_all=S_max_arr,
            sum_sq_all=sum_sq_arr,
            n_clusters_all=n_clusters_arr,
            sum_s_log_s_all=sum_s_log_s_arr,
        )
        print(f"  Curves saved to {npz}")

    return result


def main():
    ap = argparse.ArgumentParser(
        description="E_8 percolation with implicit adjacency (v4.2 with entropy estimators)",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ap.add_argument("--L", type=int, nargs="+", default=[4])
    ap.add_argument("--trials", type=int, default=32)
    ap.add_argument("--workers", type=int, default=None)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--n-bins", type=int, default=10000)
    ap.add_argument("--output", type=str, default="e8_percolation_v42_results.json")
    ap.add_argument("--save-curves", action="store_true")
    args = ap.parse_args()

    workers = args.workers if args.workers else min(args.trials, cpu_count())

    print(f"\nPlatform: {platform.system()} ({platform.platform()})")
    print(f"CPUs detected: {cpu_count()}  |  Workers used: {workers}")
    print(f"Numba: {'available' if HAS_NUMBA else 'NOT AVAILABLE'}")
    print(f"Python: {sys.version.split()[0]}")

    all_results = []
    for L in args.L:
        out = args.output.replace(".json", f"_L{L}.json") if len(args.L) > 1 else args.output
        r = run_single_L(L, args.trials, workers, args.seed, args.n_bins, out, args.save_curves)
        all_results.append(r)

    if len(all_results) >= 2:
        print("\n" + "=" * 72)
        print("FINITE-SIZE SCALING SUMMARY (5 estimators)")
        print("=" * 72)
        print(f"\n  {'L':>3} {'N':>14} {'1/pc_chi':>10} {'1/pc_dS':>10} {'1/pc_S1':>10} {'1/pc_S2':>10} {'1/pc_S3':>10}")
        for r in all_results:
            print(f"  {r['L']:>3} {r['N']:>14,} "
                  f"{1/r['pc_chi']:>10.2f} {1/r['pc_dS_max']:>10.2f} "
                  f"{1/r['pc_S1']:>10.2f} {1/r['pc_S2']:>10.2f} {1/r['pc_S3']:>10.2f}")

        Ns = np.array([r["N"] for r in all_results], dtype=float)
        if len(Ns) >= 2:
            x = Ns ** (-1/3)
            A = np.vstack([x, np.ones(len(x))]).T
            fss_results = {}
            print(f"\n  FSS extrapolation p_c(N) = p_inf + a/N^(1/3):")
            for key, label in [("pc_chi", "chi peak"),
                               ("pc_dS_max", "dS_max"),
                               ("pc_S1", "S1 (Shannon-site)"),
                               ("pc_S2", "S2 (Shannon-count)"),
                               ("pc_S3", "S3 (Renyi-2)")]:
                pcs = np.array([r[key] for r in all_results])
                slope, intercept = np.linalg.lstsq(A, pcs, rcond=None)[0]
                fss_results[f"fss_{key}_infinity"] = float(intercept)
                fss_results[f"fss_{key}_slope"] = float(slope)
                print(f"    {label:>22}: pc(inf) = {intercept:.6f}   1/pc(inf) = {1/intercept:.2f}")
            fname = args.output.replace(".json", "_FSS.json")
            with open(fname, "w") as f:
                json.dump({
                    "all_results": all_results,
                    **fss_results,
                }, f, indent=2)
            print(f"\n  Saved FSS to {fname}")


if __name__ == "__main__":
    import multiprocessing as _mp
    _mp.freeze_support()
    main()
