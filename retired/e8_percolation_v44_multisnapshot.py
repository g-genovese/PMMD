...
def _newman_ziff_kernel_v43_po(L, order, typeA, typeB_h0, typeB_h1, n_bins,
                                snapshot_bond_idxs):
    """v4.4 kernel: v4.3 + MULTI-SNAPSHOT. One sweep; parent[] is copied at each
       (sorted) target bond index in snapshot_bond_idxs.
       - activation_time[v]: bond-index i when v was first activated (-1 if never)
       - parent_snapshots[j, v]: copy of parent[v] at snapshot_bond_idxs[j]
       Returns (S_max, sum_sq, n_clusters, sum_s_log_s, activation_time, parent_snapshots).
    """
    L8 = L ** 8
    N_idx = 2 * L8
    N_valid = order.shape[0]
    ...
    n_snaps = snapshot_bond_idxs.shape[0]
    parent_snapshots = np.full((n_snaps, N_idx), -1, dtype=np.int32)
    n_taken = 0
    ...
    for i in range(N_valid):
        ...
        # MULTI-SNAPSHOT: copy parent[] at each target bond index reached this step
        while n_taken < n_snaps and i >= snapshot_bond_idxs[n_taken]:
            for k in range(N_idx):
                parent_snapshots[n_taken, k] = parent[k]
            n_taken += 1
    ...
    # Edge case: snapshots with bond_idx >= N_valid get the final state
    while n_taken < n_snaps:
        for k in range(N_idx):
            parent_snapshots[n_taken, k] = parent[k]
        n_taken += 1

    return (S_max_bin, sum_sq_bin, n_clusters_bin, sum_s_log_s_bin,
            activation_time, parent_snapshots)
...
def _worker(args):
    (L, seed, typeA, typeB_h0, typeB_h1, n_bins,
     track_po, snapshot_bond_idxs, target_ps, save_po_path, trial_idx) = args
    t0 = time.time()
    order = _build_order(L, seed)

    if track_po:
        snap_arr = np.asarray(snapshot_bond_idxs, dtype=np.int64)
        (S_max, sum_sq, n_clusters, sum_s_log_s,
         activation_time, parent_snapshots) = _newman_ziff_kernel_v43_po(
            L, order, typeA, typeB_h0, typeB_h1, n_bins, snap_arr)

        # One small per-trial NPZ per snapshot p (filtered to vertices activated
        # by that snapshot's bond index).
        if save_po_path is not None:
            for j in range(len(snap_arr)):
                sbi = int(snap_arr[j])
                mask = (activation_time >= 0) & (activation_time <= sbi)
                av = np.where(mask)[0].astype(np.int64)
                at_compact = activation_time[av].astype(np.int32)
                ps_compact = parent_snapshots[j, av].astype(np.int32)
                tag = f"_p{target_ps[j]:.6f}_trial{trial_idx:04d}.npz"
                trial_path = save_po_path.replace('.npz', tag)
                np.savez_compressed(
                    trial_path, seed=seed, snapshot_bond_idx=sbi,
                    target_p=np.float64(target_ps[j]),
                    activated_vertices=av, activation_time=at_compact,
                    parent_snapshot=ps_compact,
                    n_total_activated_at_snap=np.int64(len(av)), L=np.int32(L))
        return (S_max, sum_sq, n_clusters, sum_s_log_s, time.time() - t0, True)
    else:
...
def run_single_L(L, trials, trial_start, workers, seed_base, n_bins, output,
    ...
    if track_po:
        print(f"  Target p(s) for parent snapshot: {target_p}")
        if save_po_data:
    ...
    # Determine snapshot bond indices from target_ps (site percolation: p = n_active/N)
    N_valid_est = N
    target_ps = sorted(target_p) if isinstance(target_p, (list, tuple)) else [target_p]
    snapshot_bond_idxs = [int(round(p * N_valid_est)) for p in target_ps]
    if track_po:
        print(f"  multi-snapshot target_ps = {target_ps}")
        print(f"  snapshot_bond_idxs = {snapshot_bond_idxs} (= p * N={N_valid_est:,})")
    ...
    work = [(L, seeds[i], typeA, typeB_h0, typeB_h1, n_bins,
             track_po, snapshot_bond_idxs, target_ps, save_po_path, trial_start + i)
    ...
    if track_po:
        print(f"    PARTIAL ORDER: snapshots taken at bonds {snapshot_bond_idxs}")
        if save_po_data:
    ...
    output_data = {
        ...
        "target_ps": (sorted(target_p) if isinstance(target_p, (list, tuple)) else [target_p]) if track_po else None,
        "snapshot_bond_idxs": snapshot_bond_idxs if track_po else None,
...
def main():
    ...
    ap.add_argument("--target-p", "--target-ps", type=float, nargs="+",
                    default=[0.005800], dest="target_p",
                    help="one or more p values for parent snapshots "
                         "(multi-snapshot taken in a single sweep)")
